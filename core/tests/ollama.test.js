...
describe('callModel with OLLAMA_FIXTURE', () => {
  ...
  test('OLLAMA_FIXTURE=1 defaults to tool_call and the fixture contains at least one tool-call turn', async () => {
    process.env.OLLAMA_FIXTURE = '1';
    global.fetch = jest.fn(() => { throw new Error('fetch must not be called when OLLAMA_FIXTURE is set'); });
    const fixture = require('../fixtures/ollama/tool_call.json');
  ...
  test('OLLAMA_FIXTURE=direct_answer replays the text-only fixture', async () => {
    ...
    expect(typeof out.choices[0].message.content).toBe('string');
    expect(out.choices[0].message.content.length).toBeGreaterThan(0);
    expect(out.choices[0].message.tool_calls).toBeFalsy();
  });
  ...
  test('OLLAMA_FIXTURE=tool_then_text replays the multi-call variant', async () => {
    ...
    expect(t0.choices[0].message.tool_calls?.length).toBeGreaterThan(1);
  });
