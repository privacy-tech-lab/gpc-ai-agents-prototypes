 ...
 * caller's executeToolFn — that is where Architecture E routes the inferred
 * attributes through the firewall or the engine. The model only ever sees what
 * executeToolFn returns.
 */
    ...
    if (!msg.tool_calls?.length) {
      if (toolChoice === 'auto') {
        ...
        if (!finalResponse && turn < maxTurns - 1) {
          messages.push({ role: 'user', content: 'Please give the user a brief answer.' });
          continue;
        }
        break;
      }
