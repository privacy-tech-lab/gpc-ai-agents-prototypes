/**
 * GPC run using the proposed dedicated field: gpc=1, signal carried in
 * Message.privacySignals instead of Message.metadata.
 * Search and synthesis still run; all storage operations are blocked at Layer 4.
 */

const fs   = require('fs');
const path = require('path');
const { handleRequest } = require('../proposal-dedicated-field/orchestrator.js');

const OUTPUT = path.join(__dirname, '..', 'output', 'gpc_dedicated_field_result.json');

async function main() {
  const timing = [];

  console.log('Running GPC run with dedicated field (GPC on)...\n');
